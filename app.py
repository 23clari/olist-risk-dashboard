...
st.markdown("## 🤖 Predictive Model Center")
st.markdown("Two models help Olistic operations: one predicts delivery delay (regression), the other predicts bad reviews (classification).")
...
# === Métricas visuales
m1, m2 = st.columns(2)
m1.markdown(f"<div class='card'><h2>{rmse:.2f} days</h2><p>RMSE (Delivery Delay)</p></div>", unsafe_allow_html=True)
m2.markdown(f"<div class='card'><h2>{auc_score:.3f}</h2><p>AUC (Bad Review Classifier)</p></div>", unsafe_allow_html=True)
...
xgb_auc = roc_auc_score(y_test_xgb, xgb_late_proba)
xgb_f1 = f1_score(y_test_xgb, (xgb_late_proba > 0.5).astype(int))

m1, m2 = st.columns(2)
m1.markdown(f"<div class='card'><h2>{xgb_auc:.3f}</h2><p>AUC (Late Delivery Classifier)</p></div>", unsafe_allow_html=True)
m2.markdown(f"<div class='card'><h2>{xgb_f1:.2f}</h2><p>F1 Score</p></div>", unsafe_allow_html=True)
...
tab1, tab2, tab3 = st.tabs([
    "📦 Delivery Delay Accuracy",
    "❗ Review Risk by Delay",
...
with tab2:
    ...
    delay_bins = pd.cut(
        ...
        labels=["Very Late", "Late", "Slightly Late", "On Time", "Slightly Early", "Early", "Very Early"]
    )
    delay_risk_df = pd.DataFrame({
        "delay_segment": delay_bins,
        "risk": y_pred_proba
    }).groupby("delay_segment")["risk"].mean().reset_index()

    fig2 = px.bar(
        delay_risk_df, x="delay_segment", y="risk",
        title="Bad Review Risk by Delivery Timing",
        labels={"risk": "Avg Predicted Risk"},
